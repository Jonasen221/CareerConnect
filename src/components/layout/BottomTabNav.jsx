...
import { LayoutDashboard, Zap, Calendar, MessageCircle, Star, Phone, User, X, ChevronRight, FolderKanban, Sparkles, Settings as SettingsIcon } from 'lucide-react';
import { FEATURE_PROJECTS } from '@/lib/featureFlags';

const MAIN_TABS = [
  { label: 'Home', icon: LayoutDashboard, page: 'StudentDashboard' },
  { label: 'Jobs', icon: Zap, page: 'JobSwipe' },
...
export default function BottomTabNav({ currentPageName, bottomOffsetPx = 0 }) {
  ...
  useEffect(() => {
    ...
    if (main && scrollPositions[currentPageName] !== undefined) {
      main.scrollTop = scrollPositions[currentPageName];
    }
  }, [currentPageName]);
  ...
  const handleTabPress = (page) => {
    ...
    if (currentPageName === page) {
      navigate(createPageUrl(page), { replace: true, state: { reset: Date.now() } });
